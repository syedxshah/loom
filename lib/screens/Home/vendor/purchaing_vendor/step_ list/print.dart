...
class _PrintScreenState extends State<PrintScreen>
  ...
  void _skipPrint(Map<String, dynamic> item) {
                ...
                Expanded(
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      String clothName = widget.clothName;
                      double pricePerMeter = double.tryParse(item['price_per_meter']?.toString() ?? '0') ?? 0;

                      final allStock = await SperatevenderlistInPurchaing.getInstance.getVendorStock(widget.vendorName);
                      int existingCount = allStock.where((s) => s['suid'].toString().startsWith(item['uid'].toString()) && s['condition'] == 'print').length;
                      String suffix = String.fromCharCode(65 + (existingCount % 26));
                      String newSuid = "${item['uid']}$suffix";

                      Map<String, dynamic> newItem = Map.from(item);
                      newItem['id'] = DateTime.now().millisecondsSinceEpoch.toString() + "_phold";
                      newItem['vendor_name'] = selectedVendor;
                      newItem['meter'] = send;
                      newItem['suid'] = newSuid;
                      newItem['type'] = 'p_hold';
                      newItem['condition'] = 'print';
                      newItem['p_or_r'] = 'expense';
                      ...
                      await Bvendername.getInstance.addLedgerEntry(selectedVendor!, {
                        'name': "${widget.clothName} (Sent to Print)",
  ...
  void _showReceiveFromPrintDialog(Map<String, dynamic> item) {
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    if (received == originalSentMeter) {
                      ...
                      remainingData['total'] = (originalSentMeter - received) * originalPrice;
                      await SperatevenderlistInPurchaing.getInstance.updateStock(
                        item['id'].toString(),
                        remainingData,
                        widget.vendorName,
                      ...
                      Map<String, dynamic> doneData = Map.from(item);
                      doneData['id'] = DateTime.now().millisecondsSinceEpoch.toString() + "_pdone";
                    ...
                    Map<String, dynamic> khariData = Map.from(item);
                    khariData['id'] = DateTime.now().millisecondsSinceEpoch.toString() + "_khari";
                    khariData['meter'] = received;
                    khariData['price_per_meter'] = finalPricePerMeter;
                    khariData['total'] = totalCashAmount;
                    khariData['suid'] = newEntitySuid;
                    khariData['condition'] = 'khari';
                    khariData['type'] = 't_k_hold';
                    khariData['date'] = DateFormat('dd-MM-yyyy').format(DateTime.now());
                    ...
                    await Bvendername.getInstance.addLedgerEntry(item['vendor_name'] ?? "", {
                      'name': "${widget.clothName} (Received from Print)",
                      'date': DateFormat('dd-MM-yyyy').format(DateTime.now()),
                      'uid': item['suid'],
                      'totalmeter': received,
                    ...
                    await KHisabDB.getInstance.addHisab(
                      name: "${widget.clothName} Printing",
        ...
        return Container(
          ...
          child: ListTile(
            ...
            leading: Container(
              ...
              child: Icon(
                isTransfer ? Icons.warehouse_rounded : Icons.print_rounded,
            ...
            onTap: () {
              if (isTransfer) {
                ...
                if (item['type'] == 'p_done') {
                  ...
                  _showReceiveFromPrintDialog(item);
                }
              }
            },
            trailing: isTransfer
